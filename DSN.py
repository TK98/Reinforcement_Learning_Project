...
class SARSANetwork(nn.Module):
    ...
    def compute_targets(self, rewards, next_states, next_actions, dones):
        """
        ...
        # YOUR CODE HERE
        next_q_vals = self.compute_q_vals(next_states, next_actions)

        if dones.dtype != torch.uint8:
            adjusted_dones = torch.ones(dones.size(), dtype=torch.uint8)
            adjusted_dones[dones == True] = 0
        else:
            adjusted_dones = (dones - 1) * -1  # Switches 0 and 1 around

        return rewards + adjusted_dones * self.discount_factor * next_q_vals
